...
function normalizarStatus(status) {
  return STATUS_PERMITIDOS.includes(status) ? status : null;
}
...
export async function listarPedidosAdmin(req, res) {
  const status =
    req.query.status === "todos"
      ? null
      : normalizarStatus(req.query.status);
  ...
  if (busca) {
    ...
    filtros.push(`
      (
        LOWER(COALESCE(p.numero_pedido::text, '')) LIKE $${parametros.length}
        OR LOWER(c.nome) LIKE $${parametros.length}
        OR LOWER(c.email) LIKE $${parametros.length}
      )
    `);
  }
    ...
    const result = await pool.query(
      ...
      SELECT
        ...
        COALESCE(
          ...
          ) FILTER (WHERE ip.pedido_id IS NOT NULL),
          '[]'::json
        ) AS itens
...
export async function buscarResumoAdmin(req, res) {
  try {
    const result = await pool.query(`
      SELECT
        COUNT(*) FILTER (
          WHERE status = 'aguardando_pagamento'
        )::integer AS aguardando_pagamento,

        COUNT(*) FILTER (
          WHERE status = 'em_producao'
        )::integer AS em_producao,
    ...
    return res.json({ resumo: result.rows[0] });
  } catch (error) {
    console.error("Erro ao carregar resumo admin:", error);
    return res.status(500).json({
      erro: "Erro interno ao carregar o resumo.",
    });
  }
}

export async function atualizarStatusPedidoAdmin(req, res) {
  const pedidoId = Number(req.params.id);
  const { status, codigoRastreio, urlRastreio } = req.body;
  ...
  if (!statusNormalizado) {
    return res.status(400).json({ erro: "Status do pedido inválido." });
  }

  const codigoRastreioNormalizado = String(codigoRastreio || "").trim();
  const urlRastreioNormalizada = String(urlRastreio || "").trim();
  ...
  if (
    ...
    return res.status(400).json({
      erro: "Informe o código de rastreio para marcar o pedido como enviado.",
    });
  }

  if (
    urlRastreioNormalizada &&
    !/^https?:\/\//i.test(urlRastreioNormalizada)
  ) {
    return res.status(400).json({
      erro: "O link de rastreio precisa começar com http:// ou https://.",
    });
  }
  ...
  try {
    const result = await pool.query(
      ...
      SET
        ...
        codigo_rastreio = CASE
          WHEN $1::varchar = 'enviado' THEN $2::varchar
          ELSE codigo_rastreio
        END,

        url_rastreio = CASE
          WHEN $1::varchar = 'enviado' THEN $3::text
          ELSE url_rastreio
        END,

        enviado_em = CASE
          WHEN $1::varchar = 'enviado'
            THEN COALESCE(enviado_em, NOW())
          ELSE enviado_em
        END,

        entregue_em = CASE
          WHEN $1::varchar = 'entregue'
            THEN COALESCE(entregue_em, NOW())
          ELSE entregue_em
        END,
    ...
    if (statusNormalizado === "enviado") {
      resultadoEmail = await tentarEnviarEmailDoPedido({
        pedidoId,
        evento: "enviado",
      });
    }
    ...
    console.error("Erro ao atualizar pedido pelo admin:", {
      name: error?.name,
